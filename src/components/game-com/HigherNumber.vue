                    ...
                    <div class="p-2 lg:w-1/3 md:w-1/2 w-full truncate" v-for="(item,index) in this.$store.getters.getPlayerNum" :key="index">
                        <div class="h-full flex items-center border-gray-200 border p-4 rounded-lg">
                            ...
                            <div class="flex-grow border-l-4 border-sky-700">
                                <h2 class="text-gray-900 text-6xl title-font font-medium animate-answer-animation" v-if="this.randomList[index]">{{this.randomList[index]}}</h2>
...
export default {
    ...
    methods:{
        ...
        setRandomInt(){
            ...
            if(this.randomList.length < this.$store.getters.getPlayerNum){
                this.randomList.push(reandomInt)
            }
        }
    },
}
